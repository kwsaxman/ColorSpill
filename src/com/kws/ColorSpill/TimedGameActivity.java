 ...
 * @version Apr 29, 2012
 */
public class TimedGameActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		model = new TimedGame(10, 10, 30000);
		view.setModel(model);
	...
	private void buttonClick() {
		status.setText(Long.toString((model.getMaxTime() - model
				.getElapsedTime()) / 1000)
				+ "/"
				+ (model.getMaxTime() / 1000)
				+ " Seconds");
		if (model.isGameLost()) {
	...
	public void onYellowClick(View theView) {
		Log.i("YellowClick", "Click");
		if (model.isGameLost()) {
			Log.i("COLORSPILL", "Game Lost");
		}
		if (model.isGameWon()) {
			Log.i("COLORSPILL", "Game Won");
		}
	...
	public void onMenuClick(View theView) {
		Intent menuIntent = new Intent(TimedGameActivity.this,
				MenuActivity.class);
		finish();
		TimedGameActivity.this.startActivity(menuIntent);
	}
